...
\vspace{1cm}
\large
Juego de Estrategia:  Game of War
...
Esto me ha llevado a comprobar el reto que supone la realización de un videojuego. El tipo de juego elegido es un juego de guerra de estrategia, en tiempo real, multijugador y online. Cada jugador se enfrentará a los demás jugadores. Cada uno de ellos dispondrá de una ciudad, la cual tendrá que desarrollar y proteger del resto de jugadores

El juego debe ser capaz de dar acceso a gran cantidad de usuarios y proporcionar soporte en múltiples idiomas. Para la realización se han escogido tecnologías que se han popularizado bastante estos últimos años, y que me gustaría poder llegar a conocer bien, como son Nodejs y Angular. Además Node proporciona altas concurrencias lo que es adecuado para el juego.
...
\begin{itemize}
 ...
 \item \textbf{Capítulo \ref{chap:implementacion}. Diseño e implementación:} este capítulo se explica el funcionamiento de la aplicación y cómo ha sido diseñada y desarrollada.
...
El objetivo de mi trabajo fin de grado consiste en desarrollar de un videojuego online de estrategia en tiempo real y multijugador al que se pueda acceder a través de un navegador.
...
% Acceso a gran cantidad de usuario.
...
%REalizacion del servidor mediante el uso de postman
...
\vspace{0.4cm}
\textbf{Directivas:} son clases que define palabras clave a usar dentro de las plantillas. Cuando angular las procesa transforma el DOM de acuerdo a lo especificado por las directivas. Pueden ser estructurales si modifican el diseño o de atributo si modifican la apariencia o comportamiento de un componente.\\
...
Angular CLI~\cite{angular:cli} es una herramienta de línea de comandos utilizada para inicializar, desarrollar y mantener aplicaciones de Angular. Facilita mucho el proceso de inicio de cualquier aplicación con Angular, ya que mediante un sólo comando puedes obtener todo el esqueleto de una aplicación. Al crear un proyecto genera también un archivo de configuración, estos valores se pueden modificar editando el archivo o por línea de comandos. También permite agregar nuevos componentes, servicios, directivas, etc.\\
...
Estas traducciones se guardan en archivos Json y ofrece la posibilidad de incluir variables en las traducciones.
...
\vspace{0.4cm}
TypeScript es un superset de JavaScript, es decir que se trata de un lenguaje basado en JavaScript. Los programas de JavaScript son programas válidos de TypeScript. Esto permite integrar JavaScript en proyectos ya existentes, sin tener que rehacer todo el proyecto en TypeScript, ya que todo el código TypeScript se compila en JavaScript nativo.\\
Algunas de las características de TypeScript son:
\begin{itemize}
  \item Pone a nuestra disposición las herramientas de JavaScript ES6 (Ecmascript 6). De esta manera se mantiene a la vanguardia con las ultimas mejoras de JavaScript.
...
Es un gestor multi-hilo lo que le permite manejar muchas tareas al mismo tiempo y ser utilizado por varias personas sin tener que esperar a que otros terminen sus consultas o procesos, lo que lo hace sumamente versátil.
...
\item Permite establecer ajustes de aplicaciones web como que puerto usar, la localización de las plantillas que se utilizan, etc.
...
Se trata de un contenedor del controlador mysqljs/mysql de Node, que envuelve las llamadas con promesas Bluebird. Es decir que mientras el controlador mysqljs/mysql utiliza callbacks, promise-msqy soporta promesas.
...
Json web token(jwt) \cite{jsonwebtoken} : es un estándar abierto para crear tokens. Un token se utiliza para validar el acceso de un usuario a un servidor, ser trata de una cadena alfanumérica que se obtiene al encriptar con una clave unos determinados datos que identifiquen al usuario.
...
\textbf{Header:} La cabecera, contiene el algoritmo de encriptación (alg, comúnmente es el algoritmo HMAC SHA256) y el tipo de token (typ).
{\footnotesize
\begin{verbatim}
    {
        "alg": "HS256",
...
\textbf{Payload:} Contiene atributos que definen nuestro token. Algunos de los atributos estándar de un token son:
iat, indica cuando fue creado; jti, se trata de un identificador único para cada token; exp, indica el tiempo de expiración del token y se utiliza una fecha para validar si el token ha expirado y obligar al usuario o volver a autenticarse.

\textbf{Signature:} La firma se genera utilizando las dos primeras partes, la cabecera y el contenido, en base64 y una clave secreta, key, que se utiliza para la encriptación.
...
A pesar de ser un estándar a cargo del Consorcio WWW, fue la asociación WHATWG, formada por Apple, Opera y Mocilla; la que publico el primer borrador de HTML5 cuando W3C decidió dejar de evolucionar HTML. Más tarde W3C y WHATWG trabajaron juntos durante varios años en el desarrollo de HTML5, hasta que se separaron debido a sus diferentes objetivos, W3C quería publicar una versión terminada mientras que WHATWG quería seguir trabajando, manteniendo HTML5 en constante evolución.\\
...
Con CSS se consigue separa el contenido y el formato, dentro del mismo documento o separándolo en una hoja de estilo. Esto permite reutilizar una misma hoja de estilo en diferentes documentos y modificar por completo la estética de la página sin tener que modificar el contenido.
...
Bcript genera un hash combinando la información que se desea encriptar con un valor llamado Salt, que son bytes aleatoriamente generados. De este modo al encriptar dos veces el mismo valor nunca generará el mismo hash.
...
Bcrypy permite elegir el valor de saltRound, utilizado para calcular el campo Salt. Cuanto mayor sea este número más segura será la encriptación, pero mayor será el coste de procesamiento de los datos y más tiempo tarda en encriptar. Es decir, que tenemos la seguridad frente a ataques contra la espera del usuario. Por defecto, el valor de saltRound 10.

Ofrece también herramientas para comparar un valor con otro encriptado. Para realizar la comparación, en vez de desencriptar el campo encriptado, como hacen otros sistemas, Bcrypy encripta el valor utilizando el valor Salt que va incorporado en el hash.
...
\section{Cors}
\label{sec:cors}
Cors (Cross-origin resource sharing) es un mecanismo para permitir acceder a recursos de un servidor desde un origen distinto al que pertenece, lo cual no esta permitido según la política de seguridad del mismo origen.
...
El proyecto consta de un cliente y un servidor independientes que interactúan entre ellos. El cliente está desarrollado utilizando Angular 7. Está escrito en TypeScript y se utiliza además HTML, CSS3 y Bootstrap para la visualización de las vistas. Por otro lado, para el servidor se ha utilizado Nodejs pero está igualmente programado en TypeScript. La aplicación requiere además de una base de datos, para la que se ha utilizado Mysql.

Para que el cliente y el servidor puedan comunicarse, es decir, para que el cliente pueda acceder a los recursos ofrecidos por el servidor es necesario la utilización de CORS, ya que se encuentran en dominios diferentes y, como hemos explicado antes, esto no está permitido por motivos de seguridad.\\
En la siguiente figura podemos ver la arquitectura de la aplicación~\ref{fig:arquitectura}.
...
Cuando un usuario interactúe con la aplicación cliente a través de un navegador, esta hará peticiones HTTTP solicitando determinados recursos al servidor. El servidor accederá a la base de datos y respondera con un Json, que el cliente tratará y mostrará. Por ejemplo, cuando un usuario acceda a la URL /buildings, el cliente tendrá que mostrar los edificios de ese usuario, de modo que hará una petición GET a la ruta /api/buildings y el servidor enviará un Json con el modelo de datos de los edificios.
...
El servidor, realizado con Nodejs, gestiona eventos de manera asíncrona, como hemos explicado. 
...
Async/await nos permite ejecutar las promesas y esperar el resultado de forma síncrona, sin la necesidad de los bloques then y catch.
...
Los edificios tiene diferentes niveles y cada uno de estos niveles está especificado en la tabla \textit{buildings\_levels}: se detallan los recursos y el tiempo necesarios para mejorar el edificio, las mejoras que se producen al subir de nivel el edificio (field\_toIncrease1 y field\_toIncrease2), etc.
...
Cada niveles está especificado en la tabla \textit{research\_levels}: los recursos y el tiempo necesarios para mejorar el edificio, cuándo estará desbloqueada la investigación para poder mejorarla y el poder que proporcionará. La clave de la tabla está formada por los campos id\_res y level.


\begin{figure}[H]
  \centering
  \includegraphics[width=15cm, keepaspectratio]{img/tablasUsuarios}
  \caption{Tablas con informacion de usuarios }
...
En el se especifica, por ejemplo, a que versión de JavaScript tiene que traducirse TypeScript. En mi caso ECMAScript 6,  ``target'': ``es6''.
...
Para establecer la conexión se la dan los parámetros de la base de datos: en que host está instalado, el usuario, la contraseña, en que puerto está instalado, etc., estos parámetros están definidos en el archivo \textbf{keys.ts}.

\vspace{0.4cm}
El archivo \textbf{index.ts} es el archivo principal de la aplicación. En el configuramos el servidor de Express: se le asigna un puerto, en el caso de que haya un puerto definido en el sistema será el que utilice; se habilita el uso de Json en las peticiones HTTP; se configura CORS, esta configuración se ha hecho de tal manera que solo acepte peticiones del origen de mi cliente; se establecen las rutas y se inicia el servidor.
...
\item Cuando el token es invalido y el cliente recibe un código de error, este lo elimina del LocalStorage e indica al usuario que no está logueado.
\end{itemize}
...
Para validar el token se extrae de la cabecera textit{Authorization} de la petición HTTP y se valida utilizando la función \textit{verify} de jwt. Si el token no iba en la cabecera, si la sesión ya ha expirado o es un token invalido, el servidor devuelve un error 401 al cliente. Si es válido continua con la petición del cliente.
...
\begin{itemize}
  ...
  \item \textbf{ get api/profile/enemyList :} recupera una lista de 30 usuarios de la tabla \textit{profile} (partidas de usuarios). Para hacer la select de esa lista primero se generan identificadores de usuarios que necesitará para la busqueda, de la siguiente manera: Se recuperan de la tabla reports los usuarios que han atacado al jugador que realiza la petición, si el número total de estos usuarios es inferior a 30, se complementa la lista con identificadores de usuario generados aleatoriamente entre el total de usuarios. 
...
Al tratarse de una aplicación SPA no se recarga la página en ningún momento. La carga de datos es completamente asíncrona con el servidor y ademas este tipo de web son completamente dinámicas, es decir, que el cambio entre las diferentes urls de la web es completamente instantáneo, dinámico y reactivo. 

\vspace{0.5cm}
Angular se compone de componentes, servicios, módulos y directivas. La estuctura general del cliente es la siguiente:
...
\begin{itemize}
 ...
 Los componentes building, research y troops, son hijos de los componentes buildings-list, research-list y troops-list respectivamente. Es decir que estos componentes están completamente relacionados y se comunican la información constantemente entre ellos.
...
\subsection{Uso de multiples idiomas}
\label{subsec:cliente.idiomas}

Como hemos dicho, dentro de la carpeta i18n tenemos un Json para cada idioma en el que se quiera mostrar la aplicación. Estos Json incluyen todo el contenido de las vistas que quiera ser traducido. Y todos ellos deben tener el mismo contenido. Un ejemplo de uso es el siguiente:
{\footnotesize
\begin{verbatim}
Archivo es.json:
{
...
En la tercera pestaña se muestran las estadísticas y mejoras relacionadas con la parte militar, aquello relacionado con las tropas, el ataque, la defensa.
...
Podemos ver como la parte de arriba de la página se mantiene, la navegación, que como hemos dicho es fija.
...
Además, solo es posible poner en construcción un único edificio a la vez. Cuando se está construyendo un edificio aparece un mensaje en la parte superior de la lista con el edificio en cuestión y una cuenta atrás para su finalización. Esta cuenta atrás aparece también dentro del detalle del edificio.
...
En el lado derecha, controlado por el componente troops, aparece el detalle de una tropa, una vez que se ha pulsado sobre ella.
...
En el lado izquierdo aparece una lista con 30 jugadores rellena con jugadores que te hayan atacado en algún momento y el resto jugadores aleatoriamente generados.
...
La sexta y ultima opción del menú, Informes, redirige a la ruta /reports, controlada por el componente reports.
...
El sistema de batalla funciona por rondas. En cada ronda de la batalla hay un ataque y un contraataque, es decir, primero el jugador 1 ataca al jugador 2 y después el jugador 2 pasa a ser el atacante y el jugador 1 el defensor. En los dos partes de la ronda ambos jugadores cuentan con todas las tropas con las que iniciaron la ronda. Los soldados muertos o heridos no se restan hasta terminada la ronda.

El ataque y el contraataque se realiza de la misma manera. Cada tropa del jugador atacante ataca a cada tropa del defensor en proporción a las tropas del defensor, así dañamos equitativamente a todas las tropas. Sin importar nivel ni tipo de tropa. Esto puede verse en la figura~\ref{fig:repartoSoldados}.
...
Este reparto de tropas puede realizarse ya que en la formula de cálculo del daño no se basa en el número de defensores si no en el número de atacantes, por lo que a mismo tipo de tropas, si dividimos el resultado sería el mismo que si no lo hiciéramos. Dividimos por dos razones, primero porque cada tipo de tropa tendrá diferente defensa, y en segundo lugar, cada tropa tiene un tipo de tropa a la que es más vulnerable.
...
El modificador 0.03 se ha elegido para ajustar el crecimiento del daño según la diferencia de fuerza. Si este valor es más grande a poca diferencia de fuerzas el daño podría ser exagerado. Cuanto más pequeño sea este valor mayor margen tendremos en diferencia de fuerzas antes de que el valor se dispare de forma aplastante (Es lógico que una tropa de 250 de ataque aplaste sin problemas a otra de 80 defensa, incluso de forma exagerada... pero sino, se puede controlar cambiando este parámetro).

\vspace{0.4cm}
La precisión de la tropa es número, entre 0 y 1, que indica que puntería tiene la tropa. A mayor nivel de la tropa mejor puntería.

\vspace{0.4cm}
Suerte, incluimos un factor suerte, que puede hacer bajar el rendimiento de la tropa o aumentarlo para causar más daño, hacemos que el daño final esté entre el 80\% y el 120\% en función de un número aleatorio.
...
El jugador defensor de la batalla (no el de la ronda), cuenta ademas con un suplemento de defensa.
...
Como hemos dicho al finalizar la ronda se restan los muertos a las tropas de cada jugador. Pero también se restan temporalmente los heridos, no podrán participar en las rondas restantes. Al finalizar el combate cada jugador recuperará las tropas heridas.

\vspace{0.7cm}

El número máximo de rondas viene dado por, o bien, un valor fijo de 250 rondas o un límite de tropas perdidas del 85\%  por parte de alguno de los jugadores.
...
Si la capacidad de carga del atacante es mayor que los recursos que puede perder el defensor se llevará todos ellos, en caso contrario, se llevará de cada tipo de recurso una cantidad realizada de manera proporcional a la cantidad de ese recurso que hay disponible y de manera que la suma de los tres tipos iguale el máximo de su capacidad de carga. Es decir que por ejemplo:
{\footnotesize
\begin{verbatim}
...
Y en estos mapas no tendría por qué haber solo las ciudades de los jugadores, o limitarnos a movernos entre ellas, podría haber otros elementos como por ejemplo mares y barcos para poder cruzar, etc.
...
Como ya he dicho también el acceso de los jugadores no está limitado, cada jugador puede tener el número de cuentas que desee, esto podía modificarse y establecer un límite de cuenta por persona.
