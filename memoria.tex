...
\LARGE
GAME OF WAR \\
Diseño y Desarrollo de una Aplicación Web con HTML5 y CSS3
\vspace{4cm}
...
\vspace{1cm}
\large
Game of War: Diseño y Desarrollo de una Aplicación Web con HTML5 y CSS3
...
Los primeros prototipos de videojuegos se remontan a la década de 1950. En la década de los 70 del siglo pasado aparecieron los primeros videojuegos dirigidos al gran público. Desde entonces la industria del videojuego ha pasado a convertirse en una de las mayores industrias del entretenimiento.
...
\item Funcionar en tiempo real: se pretende conseguir que el tiempo transcurra de manera continuada, estén activos o no los jugadores.
...
\item Contar con sencillez de uso: se espera que el juego sea fácilmente entendible por cualquier posible usuario.

\item Usar tecnologías web avanzadas: JavaScript es uno de los lenguajes más populares de los últimos años. Y ha traído consigo numerosos \emph{frameworks}, como Angular, el cual se pretende utilizar para el desarrollo junto con Node.js.
...
Para convertir la clase en componente se usa el decorador  @Component, en el que se especifica cómo se va a mostrar este componente utilizando un \emph{template}, una plantilla HTML que define una vista, y para definir su comportamiento se utilizan los atributos de la clase y sus métodos.\\
...
Para que un componente se pueda comunicar con otros sub-componentes existen dos mecanismos, para enviar datos se utilizan propiedades y para recibirlos se utilizan eventos. Para implementar esa comunicación se usan \emph{binding} (uniones).
...
Las plantillas combinan HTML con el marcado de Angular, como son las directivas y los \emph{bindings}, uniones o enlaces entre los datos de la aplicación y el DOM.
...
Angular CLI incluye además un servidor, herramientas de compilación, \emph{testing}, despliegue del proyecto, etc.
...
Express~\cite{express} es el framework más popular de Node.js y muchos otros \emph{frameworks} populares de Node.js están basados en él. Es rápido, minimalista y flexible, y proporciona todo lo necesario para crear tu servidor web sobre Node. Algunas características de Express son:
\begin{itemize}
\item Se comporta como un middleware para ayudar a administrar nuestros servidores y rutas.
\item Amplia las funcionalidades del módulo HTTP que vienen por defecto con Node.js.
...
Se trata de un contenedor del controlador MySQLjs/MySQL de Node.js, que envuelve las llamadas con promesas Bluebird. Es decir, mientras el controlador MySQLjs/MySQL utiliza \emph{callbacks}, promise-msqy soporta promesas.
...
\item Cuenta con numerosas APIs. Algunas APIs populares son:
  \begin{itemize}
...
El proyecto consta de un cliente y un servidor independientes que interactúan entre si. El cliente está desarrollado utilizando Angular 7. Está escrito en TypeScript y se utiliza además HTML, CSS3 y Bootstrap para la visualización de las vistas. Por otro lado, para el servidor se ha utilizado Node.js pero está igualmente programado en TypeScript. La aplicación requiere además de una base de datos, para la que se ha utilizado MySQL.
...
El servidor, realizado con Node.js, gestiona eventos de manera asíncrona, como se ha explicado.
Existen diferentes modelos de programación asíncrona. El modelo más común es la utilización de \emph{callbacks}, es decir, cada operación asíncrona recibe una función que incluye lo que debe hacer a continuación. Pero este método dificulta la comprensión del código.
...
La información general de cada partida se guarda en la tabla \textit{profile}: los recursos actuales del jugador, la producción de recursos, el poder, los \emph{bonus} obtenidos de velocidad de construcción, investigación y creación de tropas, los \emph{bonus} de ataque, defensa y vida, etc.
...
Para establecer la conexión se pasan los parámetros de la base de datos: en qué \emph{host} está instalado, el usuario, la contraseña, en que puerto está instalado, etc. Estos parámetros están definidos en el archivo \textbf{keys.ts}.
...
\vspace{0.4cm}
\textbf{Instalación de Node.js y NPM}
...
Si se desea elegir un usuario o contraseña diferentes habría que modificar el fichero server/src/keys.ts, donde están incluidos.
